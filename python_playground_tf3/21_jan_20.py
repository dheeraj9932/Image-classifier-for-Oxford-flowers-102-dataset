...
model.add(Conv2D(12, kernel_size=7,strides=2, activation='relu', input_shape=(image_height, image_width, 3)))
...
model.add(Conv2D(40, kernel_size=3, strides=2, activation='relu'))
#model.add(MaxPool2D((2, 2)))
model.add(Dropout(0.4))

model.add(Conv2D(80, kernel_size=3, strides=1, activation='relu'))
#model.add(MaxPool2D((2, 2)))
model.add(Dropout(0.4))

model.add(Conv2D(160, kernel_size=3, strides=1, activation='relu'))
#model.add(MaxPool2D((2, 2)))
model.add(Dropout(0.4))
...
Y_pred = model.predict_generator(test_generator, tst // batch_size+1)
...
class_rep = classification_report(test_generator.classes, y_pred, output_dict=True)
print(class_rep)

df = pandas.DataFrame(conf_mat).transpose()
df.to_csv(r'./dftocsv2.csv', sep='\t', encoding='utf-8', header='true')

df2 = pandas.DataFrame(class_rep).transpose()
df2.to_csv(r'./df2tocsv2.csv', sep='\t', encoding='utf-8', header='true')

# #############################################################################################
# Save the model
model.save('./checkpoints2/model_21jan20')

# Save the weights
model.save_weights('./checkpoints2/weights_21jan20')
